...
SR          = 44100
CHUNK       = 2048
STEP_MS     = CHUNK / SR * 1000   # ~46.44 ms
...
try:
    while True:
        ...
        with torch.no_grad():
            f0, conf_t, _ = pesto_model(x, convert_to_freq=True, return_activations=False)
        ...
        # ── Filtro ───────────────────────────────────────────────────────────
        if conf >= CONF_THRESH and 28.0 <= hz <= 400.0:
            pitch_buf.append(hz)
            hold  = 0
            estado = "<<<"
        else:
